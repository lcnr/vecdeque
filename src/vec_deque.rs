...
const INITIAL_CAPACITY: usize = 0; // 2^3 - 1
...
impl<T> VecDeque<T> {
    ...
    pub fn split_off(&mut self, at: usize) -> Self {
        let len = self.len();
        match at.cmp(&len) {
            Ordering::Greater => panic!("`at` out of bounds"),
            Ordering::Equal => VecDeque::with_capacity(0),
            Ordering::Less => {
...
///
/// This function returns an unspecified result if size is 0.
#[inline]
...
fn count(tail: usize, head: usize, size: usize) -> usize {
    // size is always a power of 2
    (head.wrapping_sub(tail)) & size.saturating_sub(1)
}
...
__impl_slice_eq1! { [const N: usize] VecDeque<A>, [B; N], [B; N] }
__impl_slice_eq1! { [const N: usize] VecDeque<A>, &[B; N], [B; N] }
__impl_slice_eq1! { [const N: usize] VecDeque<A>, &mut [B; N], [B; N] }
